...
- **現況（2026-07-24 更新，tooltip/chart6）**：新增滑鼠 tooltip，可測邏輯全在純函式（`tooltip.ts` 的 `buildTooltipModel`，8 例，全案 466 passed）。**未被涵蓋的 React／canvas 接線**：`ChartContainer` 的 `subscribeCrosshairMove` handler（攤平各指標 `tooltipRows()` → `buildTooltipModel` → DOM render／定位）與各指標 handle 的 `tooltipRows()` 曝露。**canvas crosshair 在 sandbox 完全無法觸發**——實測對 container 及所有子元素派送合成 `mousemove` 都不會觸發 lightweight-charts 的 `subscribeCrosshairMove`（與畫線 `subscribeCrosshairMove` 測不到同源），故 tooltip 是否真的浮出、內容是否正確、定位是否夾在容器內，都只能本機 `npm run dev` 肉眼測。本輪驗證手法：以 `javascript_tool` 確認 `.chart-tooltip` 元素已建立並接進容器、圖表已渲染（多 canvas），再**直接注入與 `renderTooltip` 同結構的 DOM 節點**、讀 `getComputedStyle` 對白底／深字／`pointer-events:none`／`z-index`／欄數做斷言（驗證 CSS 呈現，繞過 crosshair 觸發）。
