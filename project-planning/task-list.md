...
| Task | 狀態 | 優先級 | 依賴 | 交付物 |
|---|---|---|---|---|
| [data1](task-pool/data1.md) | 等待 | High | infra1 | `OhlcvBar`/`StockDataProvider` 型別 + `providerRegistry`，TS 編譯通過 |
...
| Task | 狀態 | 優先級 | 依賴 | 交付物 |
|---|---|---|---|---|
| [drawing1](task-pool/drawing1.md) | 等待 | High | chart1 | `TrendLinePrimitive` 畫線互動 spike，桌面滑鼠與行動觸控 demo 均驗證可行（含 pan/zoom 互斥、tap 觸發） |
| [drawing2](task-pool/drawing2.md) | 等待 | High | drawing1 | `DrawingController` 正式實作（點兩下決定端點），縮放/resize 後線條不跑位 |
| [drawing3](task-pool/drawing3.md) | 等待 | Medium | drawing2, chart3 | 切換股票自動清除畫線，切代號後線條消失 |
| [drawing4](task-pool/drawing4.md) | 等待 | Low | drawing2 | 多條線管理＋刪除單條線功能 |
