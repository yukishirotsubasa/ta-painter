...
| Task | 狀態 | 優先級 | 依賴 | 交付物 |
|---|---|---|---|---|
| [indicator6](task-pool/indicator6.md) | 等待 | High | - | 參數 schema 型別化（`type: number\|enum\|color`），`IndicatorParamValues` 改 `Record<string,number\|string>`，`IndicatorPanel` 依型別渲染 number/select/color；不改既有指標行為 |
...
| [share2](task-pool/share2.md) | 等待 | Medium | share1 | URL hash 還原邏輯，分享連結開新分頁貼上能完整還原畫面；還原時同步 `ChartToolbar` draft |
