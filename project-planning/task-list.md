...
| Task | 狀態 | 優先級 | 依賴 | 交付物 |
|---|---|---|---|---|
| [chart3](task-pool/chart3.md) | 完成 | High | chart2 | `ChartToolbar` 股票代號輸入/切換，輸入不同代號能正確換圖 |
...
| [drawing5](task-pool/drawing5.md) | 完成 | High | drawing2, drawing3, drawing4 | 行動觸控人工驗證（正式部署站台）已完成：拖曳畫線/縮放錨定/切股清除均正常；終點座標偏移問題（crosshair Magnet 吸附 + trackingMode 跨手勢污染，兩輪修正）已在真實裝置重測確認解決；觸控無刪除單條線 UI 的限制已記錄為技術債，暫不處理 |
