...
>
> **後續兩項改動請一併參照**：(1) 分享連結改為**按下按鈕當下才產生**，網址列不再隨操作即時同步（見下方「分享連結的產生時機」）；(2) 開啟分享連結會進入**預覽模式**，與本機設定隔離，見 [persistence.md](persistence.md)。
...
1. **掛載當下讀一次** `window.location.hash`（之後不回讀、不監聽 `hashchange`）。這一次讀取同時決定 session 模式（有合法 `s=` 即進入預覽模式），見 [persistence.md](persistence.md)。
2. `symbol` / `prov` / `range` / `indicators` 直接作為對應 `useState` 的初始值。`range` 原本每次查詢用 `lastMonthsRange(6)` 重算，為了能被連結還原改成收進 state（後來改名為 `initialRange`，因為實際載入範圍會隨捲動往前延伸，見 [data-layer.md](data-layer.md)）。
3. **線條延後還原**：解出的 `lines` 先放進 `pendingLinesRef`（型別為 `{ stockNo, lines } | null`），等**第一批 K 線資料到位**（`bars.length > 0`）才逐條 `chartRef.current.addLine()`。不能更早——`ChartContainer` 會在 `stockNo` 變動（含首次掛載）時 `clearAll()`，太早加的線會被清掉。
   - **pending 綁定股票代號（share6）**：還原前先比對 `pending.stockNo === stockNo`，不符（連結那次查詢失敗、使用者不重整就改查別支）直接丟棄，避免線被畫到不相干的標的上。effect 依賴 `[bars, stockNo]`。
4. `status: 'invalid'` 時照常載入預設畫面（並改走一般模式、讀本機設定），並在 header 顯示「分享連結無法解析（可能被截斷或改動過），已改用預設畫面」；使用者送出新代號或切換資料源後這則提示消失。

## 分享連結的產生時機

**原本**：一個 `useEffect` 在每次狀態變動時 `history.replaceState` 把目前畫面寫回 hash，「分享URL」按鈕只是複製 `window.location.href`。

**現在**：那個 effect **已整段移除**，網址列在一般操作期間保持乾淨（不再出現又長又跳動的 `#s=…`）。連結改由按鈕按下當下才組：
...
const buildShareUrl = useCallback(() => {
  const hash = formatShareHash({
    symbol: stockNo,
    prov: dataSource,
    range: { start: earliestLoaded, end: initialRange.end },  // 目前實際載入到的範圍
...
- `App.tsx` 把 `buildShareUrl` 傳給 `ShareMenu`，再透傳給 `ShareLinkButton`；按鈕內 `navigator.clipboard.writeText(buildShareUrl())`。**`buildShareUrl()` 的呼叫放在按鈕自己的 `try/catch` 內**，編碼失敗（罕見）與剪貼簿被拒共用同一條「複製失敗，請手動複製網址列」路徑。
- **`range` 送的是 `earliestLoaded` 而非 `initialRange.start`**：使用者往左捲載入了更早的資料後，分享出去的連結必須涵蓋同一段，對方打開才看得到一樣的畫面。
- 連帶好處：`pendingLinesRef` 不再需要「清空以解封 hash 同步」這個附帶職責（原本相符／不符兩條路徑都得把 ref 設回 `null`，否則網址永遠停在還原當下的狀態），現在它只單純負責延後還原。
...
| 按鈕 | 行為 | 提示 |
|---|---|---|
| 分享URL | `navigator.clipboard.writeText(buildShareUrl())`（按下當下才組連結） | 已複製分享連結／複製失敗，請手動複製網址列 |
| 複製圖片 | 截圖 → 剪貼簿，失敗退回下載 | 已複製圖片到剪貼簿／無法直接分享，已改為下載／截圖失敗，請稍後再試 |
| 分享圖片 | 截圖 → 系統分享面板，不支援或被拒退回下載 | 已分享圖片／無法直接分享，已改為下載／截圖失敗，請稍後再試 |

提示皆 2 秒後自動消失。按鈕文字帶「URL」是為了與另外兩顆圖片按鈕區隔（原名「分享」看不出分享的是什麼）。`ShareLinkButton` 的 `buildShareUrl` 為**必填 prop**（不再有預設行為可退回，避免忘了接線時靜默複製到一條沒有狀態的網址）。
...
resolvePageHeadingColor(): string
fillCanvasBackground(canvas, color): void
composeWithHeaderLabel(chartCanvas, label, style): HTMLCanvasElement
...
interface ChartScreenshotOptions {
  ...
  headerLabel?: string;         // 有值才加標題列，例 '台積電 2330'
}
```
...
截圖上方加一條標題列，讓圖片單獨傳出去時仍看得出是哪一檔股票。

- **實作位置刻意放在共用的 `takeChartScreenshotCanvas()`**，不是兩個 `canvasToPngBlob*()` —— 這正是 [`technical-debt.md`](../project-planning/technical-debt.md) 對「截圖有同步／非同步兩條產生路徑」訂下的維護守則，如此複製圖片與分享圖片兩條路徑自動得到一致的產物。
- 合成方式（`composeWithHeaderLabel`）：另開一張 `width` 相同、`height` 多出標題列高的 canvas，整張鋪底色 → `drawImage()` 把圖表貼在標題列**下方** → 繪製文字。取不到 2d context 時**退回原圖**，不讓加標題失敗連累整張截圖。
- **順序在補底色之後**：此時圖表區已不透明，標題列另取同一個底色鋪滿，兩段接縫不會有色差。`backgroundColor: null`（要求透明背景）時標題列改用 fallback 底色，否則文字會浮在透明區上不可讀。
- **尺寸依 `devicePixelRatio` 縮放**：標題列高 34、字級 17、左內距 12（皆為 CSS px）在繪製時一律乘上 `window.devicePixelRatio`。圖表 canvas 本來就是 device px 尺寸，不跟著放大的話高 DPI 螢幕上字會小到看不清。文字色取頁面 `--text-h`（`resolvePageHeadingColor()`，fallback `#f3f4f6`）。
- **`headerLabel` 的內容必須事先備妥**：`App.tsx` 另有一個依賴 `[stockNo]` 的 effect 查股票清單取得名稱存進 `symbolName`，再組成 `symbolName ? \`${symbolName} ${stockNo}\` : stockNo` 傳入。**不能等截圖當下才查清單**——分享圖片走的是同步截圖路徑（見下方「兩條路徑的 user activation」），中間插一個 `await` 就可能失去 transient user activation。查無此代號時退化成只顯示代號。
...
分享連結產生時機與截圖標題列（2026-07-24，沙盒 Chromium + `javascript_tool`；因沙盒連不到 CORS proxy，資料面以 stub `window.fetch` 回合成 K 線繞過）：

- **網址列不再隨操作變動**：送出代號、載入資料、往前補資料全程 `location.hash` 恆為空字串。
- **按下才產生連結**：stub `navigator.clipboard.writeText` 攔下內容，確認為含 `#s=` 的完整網址；以 App 自己的 `readShareHash()` 反解回 `{ symbol: '2317', range: { start: '2025-02-01', end: '2026-07-24' } }`——`start` 是**往前補過之後**的 `earliestLoaded`（初始區間起點為 `2026-02-01`），確認分享的是目前實際載入範圍。
- **截圖標題列**：攔截 `clipboard.write` 取出真正的 PNG（104,887 bytes）解回 bitmap 檢查——`devicePixelRatio` 為 1.8 時標題列高度為 `round(34 × 1.8) = 61px`，該區左上角像素為 `(22,23,29)` = `#16171d`（底色正確），區內有 1,620 個非底色像素（文字確實畫上去了；當下該代號無資料，若沒有標題列這條 strip 會是純底色）。文字邊界 `x ∈ [24,159]`、`y ∈ [17,44]`：左緣貼齊 `round(12 × 1.8) = 22` 的內距（+2 為字形左側空隙），垂直置中於 61px 標題列。標籤內容與清單查得的 `台泥 1101` 一致。

## 已知限制 / 尚未實作

- **`range` 起點會隨捲動延伸，終點固定**：`initialRange.end` 仍是分享當下的日期，因此舊分享連結打開時用的是**當初分享的區間**，不會跟著今天往後滾動；起點則是分享者當時已往前載入到的位置（見 [data-layer.md](data-layer.md)）。仍然沒有讓使用者直接指定區間的 UI。
...
- **標題列只有股名與代號**：不含日期區間、資料源或指標摘要。文字過長時不截斷也不縮排，超出畫面寬度會被裁掉（台股股名最長 6 字左右，實務上不會發生）。
- **股名取自每週更新的股票清單**：清單載入失敗或代號不在清單內時標題列只顯示代號，見 [stock-list.md](stock-list.md)。
