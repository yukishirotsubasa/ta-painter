...
export interface ChartHandle {
  deleteLine(id: string): void;
  /** 清空所有畫線；跳窗確認由呼叫端負責（見 sidebar.md）。 */
  clearAllLines(): void;
...
- 另掛一個 `touchmove` 監聽器（`{ passive: false }`），僅在**單指**拖曳中呼叫 `preventDefault()`，避免瀏覽器原生觸控捲動搶走拖曳手勢。
...
畫線模式下 `handleScroll`/`handleScale` 都是關的，但**多指觸控仍須明確排除**：

```ts
private isMultiTouch(event: TouchEvent): boolean {
  if (event.touches.length <= 1) return false;
  this.discardActiveLine();   // 連進行中的那一筆一起丟
  return true;
}
```

- `onTouchStart`／`onTouchMove` 都先過 `isMultiTouch()`，見到第二指就 `discardActiveLine()`（`detachPrimitive` 預覽線 + 清掉 `dragging`/`anchor`）後直接 return。
- **關鍵在「連已開始的拖曳一起丟」**：只是忽略新的 touch 不夠——第二指落下後 crosshair 會跟著跑，`onCrosshairMove` 會把它當成同一次拖曳的終點持續更新預覽線，放開時 `endDrag()` 就定案出一條使用者沒打算畫的歪線。丟棄後 `dragging === false`，後續的 crosshair move 與 `touchend` 自然都成為 no-op，不需要另外記「這次手勢已作廢」的狀態。
- 多指時也**不**呼叫 `preventDefault()`，手勢原樣交還給瀏覽器／圖表。
- 瀏覽器層級的捲動、慣性滑動與雙擊縮放另由 CSS 擋（見下方 `DrawingToolbar` 一節的 `.chart-container-drawing`）：`touchmove` 的 `preventDefault()` 只在真的拖曳中才呼叫，起點落在量能 pane（`pointFromClientXY` 回 `null`，不進入拖曳）之類的情況擋不住整頁跟著滑。

單元測試（`drawingController.test.ts`，fake `TouchEvent` 只需 `touches.length` + 第一指座標 + `preventDefault`）涵蓋：單指拖曳照常畫線／第二指落在拖曳中會 detach 預覽線且放開不定案／雙指 `touchstart` 完全不起拖曳／`preventDefault` 只在單指拖曳中被呼叫。
...
- **三個呼叫端共用同一個 `clearAll()`**：切換股票（`stockNo` effect）、側邊欄的「清空所有」按鈕（經 `ChartHandle.clearAllLines()`，見 [sidebar.md](sidebar.md)）、退出預覽模式（見 [persistence.md](persistence.md)）。沒有為了按鈕另寫一份清除邏輯。
...
<DrawingToolbar
  ...
  compact={breakpoint === 'mobile'}   // responsive2：行動版精簡工具列
/>
```

- **整組用 `<fieldset>` + `<legend>畫線</legend>` 框起來**（responsive2 依實測回饋加上）：使用者回報「並排的色塊看不出是給畫線用的」，因此外框＋標題明確表達兩個控制項同屬一個功能，語意上也正好是一組相關控制項。畫線模式開啟時整組高亮（`.drawing-toolbar-active`：accent 邊框 + 底色，legend 也轉 accent 色），不只按鈕變色。
- **畫線模式開關**（`className="drawing-toggle"`）：用 `aria-pressed` 屬性反映開關狀態。按鈕文字為「模式：開/關」（有了 legend 之後不必再重複「畫線」二字），`compact` 時為「畫線」／開啟中「畫線中」並補 `aria-label="畫線模式"`。樣式在 `DrawingToolbar.css`。
  - **`[aria-pressed='true']` 為實心 accent**（`color: var(--bg)` / `background`+`border-color: var(--accent)` + 粗體，responsive3 改）：原本的淡底（`--accent-bg`）與 fieldset 自己的 `.drawing-toolbar-active` 底色太接近，一眼看不出模式開著；模式開著時圖表的 pan/zoom 是失效的，狀態必須一眼可辨，否則使用者會誤以為手勢壞了。responsive3 的模式提示**只做在工具列、刻意不動圖表**（曾評估圖表外框＋角落浮標，使用者選擇不加畫面元素）。
- **選色器**：`<input type="color" id="drawing-toolbar-color">` + 「線色」label（`title="畫線前選色，畫出後不可更改"`），視覺尺寸比照 `IndicatorPanel` 的線色輸入（28×22px）。色塊左邊多一個 **28×16 的 SVG 線段預覽**（`<line stroke={color}>`），直接表達「這個顏色就是等一下畫出來的線」。`compact` 時「線色」文字改 `.sr-only`，色塊與預覽保留。
- `App.tsx` 的 `drawingColor` state 同時往下傳給 `DrawingToolbar`（顯示）與 `ChartContainer`（`drawingColor` prop → `setDrawingColor()`）。
- 圖表容器在 `drawingMode` 開啟時多掛 `chart-container-drawing` class（`ChartContainer.css`）：`cursor: crosshair` + `touch-action: none`（把捲動、慣性滑動與雙擊縮放整組交還給自己的 touch handler）+ `user-select: none` / `-webkit-user-select: none` / `-webkit-touch-callout: none`（長線拖到一半不被 iOS 的文字選取與長按 callout 打斷）。**非畫線模式不設** `touch-action`，圖表原生 pan/zoom 仍由 lightweight-charts 自己的 `preventDefault` 處理（該庫完全不設 `touch-action`，因此兩者不衝突）。
