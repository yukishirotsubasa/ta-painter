...
- **畫線模式開關**（`className="drawing-toggle"`）：用 `aria-pressed` 屬性反映開關狀態，`[aria-pressed='true']` 套用 `--accent`/`--accent-bg`/`--accent-border`（與 `IndicatorPanel` 既有的 accent 色系一致）+ 粗體高亮。按鈕文字為「模式：開/關」（有了 legend 之後不必再重複「畫線」二字），`compact` 時縮成「畫線」並補 `aria-label="畫線模式"`。樣式在 `DrawingToolbar.css`。
